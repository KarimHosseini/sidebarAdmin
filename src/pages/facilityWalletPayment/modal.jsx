...
import {
  ...
  EDIT_STATE_FACILITY_WALLET_PAYMENT,
  EDIT_STATE_FACILITY_WALLET_PAYMENT_CODE,
  GET_CHILD_FACILITIES,
} from "../../helpers/api-routes";

import { configReq } from "../../helpers/functions";
import { logout } from "../../redux/slices/user";
const WalletFacilityRquestModal = ({
  open,
  close,
  prevData = {},
  forEdit,
  ...
  useEffect(() => {
    if (forEdit) {
      ...
      axiosInstance
        .get(
          `${baseUrl}/${GET_CHILD_FACILITIES}?gatewayId=${prevData?.gateway}&userId=${prevData.userId}`,
          configReq(token)
        )
        .then((res) => {
          res.data.data
            ...
            .map((item) => {
              const rem = item.children.find(
                (i) => i.id === prevData.facilityId
              );
              if (rem?.remValue) setRemvalue(rem?.remValue);
            });
        })
        .catch((err) => {
  ...
  const editStatus = () => {
    ...
    axiosInstance
      .put(
        `${baseUrl}/${EDIT_STATE_FACILITY_WALLET_PAYMENT}`,
        datas,
        configReq(token)
      )
      .then((res) => {
  ...
  const setConfirmCode = () => {
    ...
    axiosInstance
      .put(
        `${baseUrl}/${EDIT_STATE_FACILITY_WALLET_PAYMENT_CODE}`,
        { id: prevData.id },
        configReq(token)
      )
      .then((res) => {
...
export default WalletFacilityRquestModal;
const STATUS = [
