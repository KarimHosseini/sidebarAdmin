...
const PIXEL_RATIO = 4;
const UploadImageHandler = ({ setData, setOpen, setPreviewH }) => {
  ...
  const [filenma, setFileNames] = useState();
      ...
      if (image !== null) {
        try {
          Resizer.imageFileResizer(
            ...
            0,
            (uri) => {
              const newn = new File([uri], `${filenma}.webp`);
              uploadImageHandler(newn);
            },
            "blob"
          );
        } catch (err) {}
  ...
  const resizedCropedImage2 = (image) => {
    if (filename.type === "image/svg+xml" || filename.type === "image/gif") {
      uploadImageHandler(image);
    } else {
      if (image !== null) {
  ...
  const handleUploadImage = () => {
    ...
    if (resizing && croping) {
      ...
      resizedCropedImage2(blob ? blob : preview);
    } else {
      resizedCropedImage2(preview);
    }
  };
  ...
  function getBase64(file) {
    ...
    reader.onload = function () {
      setPreviewH(reader.result);
    };
    reader.onerror = function (error) {
    };
  }
  const handleUploadImage2 = () => {
    uploadImageHandler(preview);
  };
  return (
    <div>
      {" "}
      ...
      >
        ...
        >
          <Tab label="  آپلود عادی" {...a11yProps(0)} />
          <Tab label="  آپلود  با امکانات" {...a11yProps(1)} />
        </Tabs>
      </Box>
      <TabPanel value={value} index={0}>
        ...
        <div className="flex w-full justify-center mt-2">
          <Dropzone
            onDrop={(acceptedFiles) => {
              ...
              setFileNames(acceptedFiles[0].name?.split(".")[0]);
            }}
        ...
        </div>    <div className="flex items-center justify-center my-3">
