...
import {
  baseUrl,
  WidgetMonthlyRegistrationByDay,
...
const WidgetMonthlyRegistrationByDays = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axiosInstance(
      `${baseUrl}/${WidgetMonthlyRegistrationByDay}?year=${years}&month=${time}`,
      configReq(token)
    )
      .then((res) => {
        setLoading(false);
    ...
    datasets: [
      {
        ...
        borderWidth: 2,
        fill: false,
        data: data.map((item) => item.males + item.females),
      },
      {
        type: "bar",
        label: "مرد",
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-sm"> گزارش ثبت نام ماهانه </span>
        {loading ? (
...
export default WidgetMonthlyRegistrationByDays;
const months = [
  "فروردین",
  "اردیبهشت",
  "خرداد",
