    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component="div"
        >
          انتخاب موارد برای تسهیم تجمیعی
        </Typography>
      )}
      {numSelected > 0 ? (
...
const ShareBluBank = () => {
  ...
  const getData = () => {
    ...
    axiosInstance
      .get(
        `${baseUrl}/${GetDataToSharingBluBank}${params ? `?${params}` : ""}`,
        configReq(token)
      )
      .then((res) => {
  ...
  const handleSharj = () => {
    ...
    axiosInstance
      .post(`${baseUrl}/${SharingBluBankList}`, selected, {
        headers: {
  ...
  if (!userPermissions?.RefahLoans?.chargeBlueBank) {
    return <NoAccess />;
  }
  ...
  return (
    <div>
      ...
      <Confirm
        message={`آیا از   تسهیم این  ${selected.length} مورد  اطمینان دارید؟`}
        close={() => setOpen(false)}
        submit={handleSharj}
        open={open}
